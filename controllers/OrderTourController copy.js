...
function getInforTour(res, result) {
    ...
    Promise.all([Promise.all(tourPromises), Promise.all(customerPromises), Promise.all(statusPromises)])
...
class OrderTourController {
    ...
    filter(req, res, next) {
        ...
        switch (id) {
            ...
            default:
                if (status == 'Tất cả' || status == undefined) {
                    OrderTour.getAllFollowCustomer(id, paging, function (result, status, mess) {
                        if (status) getInforTour(res, result);
                        else res.send(message(result, status, mess));
                    });
                } else {
                    OrderTour.findByStatusFollowCustomer(id, status, paging, function (result) {
    ...
    async getNumberOrder(req, res, next) {
        ...
        const idTour = query.idTour == undefined ? -1 : query.idTour;
        ...
        switch (idCustomer) {
            case undefined:
                ...
                break;
            default:
                OrderTour.getNumberOrderOfCustomer(idCustomer, idStatus, idTour, function (result, status, mess) {
    ...
    confirmUsing(req, res, next) {
        OrderTour.confirm(req.params.id, 12)
            .then((rs) => {
                if (rs.affectedRows != 1) res.send(message([{}], fasle, 'Cập nhật thất bại!'));
                else
                    OrderTour.getById(req.params.id)
                        .then((result) => {
    ...
    orderedTour = async (req, res, next) => {
        try {
            let [rows, field] = await connection.execute('select * from v_orderedtour where idCustomer =?', [
                req.body.idCustomer,
            ]);
