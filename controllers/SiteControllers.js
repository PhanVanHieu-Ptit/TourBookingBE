    ...
    async getOwnInfor(req, res) {
        ...
        try {
            ...
            if (req.body.role == 'customer')
                [rows, fields] = await connection.execute('call railway.sp_get_customer_by_email(?);', [email]);
            else [rows, fields] = await connection.execute('call railway.sp_get_staff_by_email(?);', [email]);
    ...
    async getNumberStaff(req, res) {
        try {
            ...
            if (req.body.role == 'admin') {
                [rows, fields] = await connection.execute('SELECT * FROM railway.v_staff_number;');
        ...
        try {
            ...
            if (type == 'destination')
                [result.destination, feild] = (
                    await connection.query('call managetour.sp_statistic_destination(?);', year)
                )[0];
            else {
                [result.revenue, feild] = (await connection.query('call managetour.sp_statistic_revenue(?);', year))[0];
                [result.person, feild] = (await connection.query('call managetour.sp_statistic_person(?);', year))[0];
