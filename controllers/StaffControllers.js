...
class StaffController {
  ...
  async addStaff(req, res) {
    try {
      ...
      await connection.execute("call railway.sp_create_account(?, ?, ?);", [
        email,
        endcodedPassword,
        "staff",
      ]);
      ...
      if (rs.response.includes("OK")) {
        if (rows.affectedRows == 1)
          return res.send(message({}, true, "Thêm mới nhân viên thành công!"));
  ...
  async updateStaff(req, res) {
    try {
      ...
      let [rows, fields] = await connection.execute(
        "call railway.sp_update_staff(?, ?, ?);",
        [name, idStatus || 5, idStaff]
      );
      if (rows.affectedRows < 1) {
        return res.send(
      ...
      if (imageUrl)
        await connection.execute("call railway.sp_update_image_staff(?, ?);", [
          imageUrl,
          idStaff,
        ]);
      [rows, fields] = await connection.execute(
        `call railway.sp_get_staff_by_id(${idStaff});`
      );
  ...
  async toggleAccountStatus(req, res) {
    try {
      ...
      if (rows[0].length == 0) {
        return res.send(
          message(rows[0], false, "Không tìm thấy nhân viên có id:" + idStaff)
        );
      }
      ...
      [rows, fields] = await connection.execute(
        "call railway.sp_update_status_staff(?, ?);",
        [newStatus, idStaff]
      );
      if (newStatus == 5)
